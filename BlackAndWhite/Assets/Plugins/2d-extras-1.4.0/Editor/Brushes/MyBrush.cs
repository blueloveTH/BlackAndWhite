...
namespace UnityEditor.Tilemaps
{
    ...
    /// </summary>
    [CustomGridBrush(true, false, false, "My Brush")]
    [CreateAssetMenu(fileName = "New My Brush", menuName = "Brushes/My Brush")]
    public class MyBrush : GridBrush
    {
        ...
        public override void Paint(GridLayout grid, GameObject brushTarget, Vector3Int position)
        {
            ...
            if (lineStartActive)
            {
                ...
                else
                {
                    foreach (var point in GetPointsOnLine(startPos, endPos, fillGaps))
                    {
                        ...
                        if ((paintPos.y % 2 == startPos.y % 2) && (paintPos.x % 2 == startPos.x % 2))
            ...
            {
                if ((position.y % 2 == startPos.y % 2) && (position.x % 2 == startPos.x % 2))
                    base.Paint(grid, brushTarget, position);
            }
            else
            {
    ...
    public class MyBrushEditor : GridBrushEditor
    {
        private MyBrush lineBrush { get { return target as MyBrush; } }
        ...
        public override void OnPaintSceneGUI(GridLayout grid, GameObject brushTarget, BoundsInt position, GridBrushBase.Tool tool, bool executing)
        {
            ...
            if (lineBrush.lineStartActive)
            {
                ...
                {
                    foreach (var point in MyBrush.GetPointsOnLine(startPos, endPos, lineBrush.fillGaps))
                    {
                        Vector3Int paintPos = new Vector3Int(point.x, point.y, position.z);
                        if ((paintPos.y % 2 == startPos.y % 2) && (paintPos.x % 2 == startPos.x % 2))
