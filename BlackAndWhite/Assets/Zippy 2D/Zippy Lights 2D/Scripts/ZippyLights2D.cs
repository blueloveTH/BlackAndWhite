...
public class ZippyLights2D : MonoBehaviour
{
	...
	public GameFlow.ObservationSet<GameObject> observations { get; private set; } = new GameFlow.ObservationSet<GameObject>();
	...
	void ScanPoints() {
		...
		for (int i = 0; i < resolution; i++) {
            ...
            #region NEW
            int count = Physics2D.RaycastNonAlloc(cacheTransform.position, qd, _hits, range, layers);

			RaycastHit2D hit = new RaycastHit2D();

			for (int j = 0; j < count; j++)
			{
				var go = _hits[j].transform.gameObject;
				if (go.layer == LayerMask.NameToLayer("Light2D Trigger"))
					currentHits.Add(go);
				else
				{
					hit = _hits[j];
			...
			if (hit) {
				// NEW
				currentHits.Add(hit.transform.gameObject);
		...
		// NEW
		observations.Overwrite(currentHits);
	}
